...
<html lang="en">
<head>
  <title>Xem Sản Phẩm Mới Về</title>
  <?php
...
<body>
  ...
  <div class='body containter-fluid' style='background-color: rgba(174, 28, 154, 0.08);'>
    <div class='container-md py-5'>
      <div class='fw-bold h5'>SẢN PHẨM MỚI VỀ</div>
      ...
      <div class="pagination d-flex justify-content-center mt-3">
        <?php
          $row_number = execute_query("SELECT COUNT(*) AS dem FROM sanpham ORDER BY MaSanPham DESC")[0]['dem'];
          $page_number = (int) $row_number/$page_length;
          if($row_number % $page_length != 0)
            $page_number++;
          for($i = 1; $i <= $page_number; $i++){
            echo "<ul class='pagination px-1'>
                <a href='xem_theo_hang_moi_ve.php?pid={$i}' class='page-link'>{$i}</a>         
              </ul>";
          }
        ?>
      </div> 
